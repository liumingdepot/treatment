  ...
  <div class="app-container">
    <el-form label-position="right" label-width="120px" :model="readerForm" :rules="rules">
      <el-form-item label="数据库源：" prop="datasourceId">
        <el-select v-model="readerForm.datasourceId"  style="width: 300px" @change="rDsChange">
          <el-option
      ...
      <el-form-item v-show="dataSource==='postgresql' || dataSource==='oracle' ||dataSource==='sqlserver'" label="Schema：" prop="tableSchema">
        <el-select v-model="readerForm.tableSchema" filterable default-first-option  style="width: 300px" @change="schemaChange">
          <el-option
      ...
      <el-form-item label="数据库表名：" prop="tableName">
        <el-select v-model="readerForm.tableName" filterable default-first-option  style="width: 300px" @change="rTbChange">
  ...
  methods: {
    ...
    getTables(type) {
      if (type === 'rdbmsReader') {
        ...
        dsQueryApi.getTables(obj).then(response => {
          if (response) {
            this.rTbList = response.sort((a,b)=>a.localeCompare(b))
          }
        })
      }
    },
    getSchema() {
      ...
      dsQueryApi.getTableSchema(obj).then(response => {
        this.schemaList = response.sort((a,b)=>a.localeCompare(b))
      })
    },
    // schema 切换
    schemaChange(e) {
